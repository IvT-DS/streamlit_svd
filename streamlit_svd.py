...
if image_url:    
    try:
        ...
        num_singular_values = st.slider("Выберите количество сингулярных чисел", min_value=1, max_value=np.min(gray_image.shape), value=singular_value)
        ...
        if st.button("Сжать изображение"):
            ...
            trunc_U = U[:, :top_k]                  # trunc_U будет содержать первые top_k столбцов матрицы U.
            trunc_sigma = sigma[:top_k, :top_k]     # trunc_sigma будет содержать верхний левый квадрат размером top_k x top_k из матрицы sigma.
            trunc_V = V[:top_k, :]                  # trunc_V будет содержать первые top_k строк матрицы V.
            ...
            st.write(f"Фото сжато на {np.round(((1 - top_k / gray_image.shape[0]) * 100), 2)} %")
        ...
        st.write("Ошибка при загрузке изображения. Пожалуйста, убедитесь, что URL ссылка корректна.")
